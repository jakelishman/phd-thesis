\newlength\calciumfigureheight\setlength\calciumfigureheight{6cm}%
...
\coordinate (D32) at (1.2\energylevellength, 0.37\calciumfigureheight);
\coordinate (D52) at (1.2\energylevellength, 0.52\calciumfigureheight);
\coordinate (P12) at (-1.2\energylevellength, 0.85\calciumfigureheight);
\coordinate (P32) at (-1.2\energylevellength, \calciumfigureheight);
\foreach \level/\split in {S12/14.0, P12/4.75, P32/9.33, D32/5.60, D52/8.40} {
    ...
    \node [anchor=north \if\dir1west\else east\fi]
          ...
          {$\if D\letter3\else4\fi\,{}^2\mathrm{\letter}_{\numer\mkern-1mu/\mkern-1mu\denom}$};
    \foreach \x in {0, ..., \numer} {
...
\path [photon=bluesb]
    ($(397.south) + (0, .15\calciumfigureheight)$)
    -- +(\energylevellength, .1\calciumfigureheight)
